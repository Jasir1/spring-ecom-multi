...
//            int newQty = cartDTO.getQty() + cart.getQty();
    ...
    public ApplicationResponseDTO minusFromCart(Long productId, CartDTO cartDTO) {
...
//        return cartList;
        ...
        return cartRepository.findByUserIdAndProductId(userService.getCurrentUser().getId(),productId);
    }
...
//        return cartList;
        ...
        return cartRepository.findByUserIdAndProduct_NameContainsIgnoreCase(userService.getCurrentUser().getId(),productName);
    }
    ...
    public ApplicationResponseDTO deleteProductFromCartByProductId(Long id) {
        User user = userRepository.findByEmail(userService.getCurrentUser().getEmail())
                .orElseThrow(() -> new ApplicationCustomException(HttpStatus.BAD_REQUEST, "USER_NOT_FOUND", "User not found"));
        Cart cart = cartRepository.findByProductIdAndUserId(id, user.getId()).orElseThrow(() -> new ApplicationCustomException(HttpStatus.NOT_FOUND, "PRODUCT_NOT_FOUND", "Product not found"));
        cartRepository.delete(cart);
        return new ApplicationResponseDTO(HttpStatus.OK, "PRODUCT_DELETED_FROM_CART_SUCCESSFULLY", "Product deleted from cart successfully!");
    }
